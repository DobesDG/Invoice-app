...
export default function InvoiceDetailsPage({ params }: { params: { id: string } }) {
    ...
    return (
     <ThemeContext.Provider value={dark}>
        <div className={`flex flex-col ${dark ? "bg-dark-purple" : "bg-gray-50"}`}>
            <Header dark={dark} setDark={setDark}/>
            <section className={`flex flex-col justify-start pt-16 items-center w-full min-h-[100vh] tracking-[-0.25px] pl-[103px] max-xl:pt-[103px] max-xl:pl-0 max-md:pb-[90px] ${dark ? "text-white" : 'text-steel-blue'}`}>
                <section className="flex flex-row justify-between w-[730px] max-xl:w-[672px] max-md:w-[87.5%]">
                ...
                <section className={`flex flex-row border-[1px] p-8 rounded-lg mb-4 w-[730px] justify-between max-xl:w-[672px] max-md:w-[87.5%] ${dark ? "bg-dark-blue border-dark-blue" : "bg-white border-white shadow-filterShadow"}`}>
                    <div className='flex flex-row items-baseline max-md:justify-between max-md:w-full'>
                        <p className='text-[12px]'>Status</p>
                        <div className={`flex flex-row justify-center items-center h-10 w-[6.5rem] rounded-md ml-8 max-md:ml-0 ${colorStatus(invoice.status)[2]}`}>
                ...
                <section className={`flex flex-col border-[1px] p-6 rounded-lg w-[730px] max-xl:w-[672px] max-md:w-[87.5%] ${dark ? "bg-dark-blue border-dark-blue" : "bg-white border-white shadow-filterShadow"}`}>
                    <section className='flex flex-row justify-between mb-8 max-md:flex-col max-md:gap-4'>
                    ...
                    <section className='flex flex-row max-md:flex-col'>
                        <div className='flex'>
                            ...
                            <div className='mr-40 max-md:mr-0'>
                    ...
                    <section className='flex flex-col justify-center align-baseline w-full'>
                        <div className={`rounded-t-lg p-8 max-md:p-6 ${dark ? "bg-light-purple border-dark-blue" : "bg-transp-white border-white text-black"}`}>
                            <table className="w-full table-fixed">
                                <thead>
                                    <tr className="text-[0.6875rem] max-md:hidden">
                                ...
                                <tbody>
                                    {invoice.item_list.map((inv, index) => (
                                        <tr key={index} className="text-[12px] font-bold">
                                            <td className="py-4 max-md:hidden">{inv.item_name}</td>
                                            <td className="py-4 text-right max-md:hidden">{inv.quant}</td>
                                            <td className="py-4 text-right max-md:hidden">${inv.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                            <tr className='md:hidden'>
                                                <tr>
                                                    <td className='py-1'>{inv.item_name}</td>
                                                </tr>
                                                <tr>
                                                    <td className='py-1'>{inv.quant} x ${inv.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        ...
                        <div className={`bg-dark-gray rounded-b-lg py-8 px-6 flex flex-row justify-between items-baseline text-white ${dark ? 'bg-dark-gray' : 'bg-light-purple'}`}>
                            ...
                            <p className='text-[20px] font-bold max-[375px]:text-[14px]'>${invoice.item_list.reduce((acc, cur) => acc + cur.quant * cur.price, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                        </div>
                    </section>
                </section>
            </section>
            <div className={`md:hidden flex justify-center items-center fixed w-full h-[80px] left-0 bottom-0 ${dark ? 'bg-dark-blue' : 'bg-white'}`}>
                <Mutate data={invoice} invoiceId={invoice._id} invoiceStatus={invoice.status}/>
            </div>
        </div>
