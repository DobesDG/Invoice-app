...
export const Input: ForwardRefExoticComponent<InputProps> = forwardRef((props: InputProps, ref:Ref<HTMLInputElement>) => {
    ...
    return (
        <div className={`flex flex-col text-xs mb-6 ${theme ? 'text-white' : 'text-blue-steel'}`}>
            ...
            <input className={`py-[15px] px-[18px] font-bold  border rounded [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${theme ? 'bg-dark-blue border-light-purple' : 'bg-white border-light-gray text-black'}`} {...InputProps} ref={ref}/>
        </div>
    );  
});